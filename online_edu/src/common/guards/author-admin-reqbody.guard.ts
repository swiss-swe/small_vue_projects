...
@Injectable()
export class AuthorAdminReqBodyGuard extends AuthGuard('access-jwt') {
  ...
  canActivate(context: ExecutionContext) {
    try {
      ...
      if (
        (admin.is_active && admin.is_admin) ||
        (admin.is_active && admin.is_author && request.body.id === admin.sub)
      ) {
        return true;
      } else {
